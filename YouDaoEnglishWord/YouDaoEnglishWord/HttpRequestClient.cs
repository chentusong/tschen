...
namespace ScanWeb
{
 ...
 User-Agent:Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36";
        ...
        /// <returns></returns>
        public string http(string url, string method, string headers, string content, Encoding contentEncode, string proxyUrl, int port)
        {
            ...
            fillProxy(request, proxyUrl, port);
        ...
        /// <returns></returns>
        public string httpPost(string url, string headers, string content, Encoding contentEncode, string proxyUrl, int port)
        {
            return http(url, "POST", headers, content, contentEncode, proxyUrl, port);
        }
        ...
        /// <param name="proxyUrl"></param>
        /// <returns></returns>
        public string httpGet(string url, string headers, string content, string proxyUrl, int port)
        {
            return http(url, "GET", headers, null, null, proxyUrl, port);
        }
        ...
        private void fillProxy(HttpWebRequest request, string proxyUri, int port)
        {
            if (!string.IsNullOrWhiteSpace(proxyUri) && port > 0)
            {
                WebProxy proxy = new WebProxy(proxyUri, port);
